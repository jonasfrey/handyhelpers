<!-- {"s_msg":"this file was automatically generated","s_by":"f_generate_markdown.module.js","s_ts_created":"Mon Mar 17 2025 12:18:41 GMT+0100 (Central European Standard Time)","n_ts_created":1742210321948} -->
...
```
# 'f_o_webgl_program'
backbuffer, last frame, automata, texture, blit, fbo
this is just a boilerplate and an example of how to access data from the last frame
which is needed for an automata for example
```javascript
            // it is our job to create or get the cavas
            let o_canvas = document.createElement('canvas'); // or document.querySelector("#my_canvas");
            // just for the demo 
...
```
## f_o_img_cached
load an image and cache it in an array
```javascript
            let a_o_img = [];
...
### load from decoded audio data
if you use for example Tonejs you can use the already decoded buffer
```javascript
...
### load from raw audio data / samples
```javascript
            let n_seconds = 10;
            let n_khz = 48000;
...
```
# 'f_flat_frontend_framework'
a minimal / lightweight / small / flat frontend framework
```javascript
            class O_person{
                constructor(
                    s_name, 
...
module that makes an image gallery
```javascript
            let f_a_v_shuffled = function(array) {
                for (var i = array.length - 1; i >= 0; i--) {
                    var j = Math.floor(Math.random() * (i + 1));
